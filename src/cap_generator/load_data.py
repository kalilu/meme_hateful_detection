...
def prepare_dataset(data='dev'):
  ...
  if data == 'dev':
    # load_dev_set (1K)
    filename = f'{PATH_TRAIN_IMAGES_TEXT}/Flickr_8k.devImages.txt'
    print(filename)
    ...
    # descriptions
    train_descriptions = load_clean_descriptions(f'{PATH_MODEL}/descriptions.txt', train)
    test_descriptions = load_clean_descriptions(f'{PATH_MODEL}/descriptions.txt', test)
    print('Descriptions: train=%d, test=%d' % (len(train_descriptions), len(test_descriptions)))

    # photo features
    train_features = load_photo_features(f'{PATH_MODEL}/features.pkl', train)
    test_features = load_photo_features(f'{PATH_MODEL}/features.pkl', test)
    print('Photos: train=%d, test=%d' % (len(train_features), len(test_features)))

  elif data == 'train':
    # load training dataset (6K)
    filename = f'{PATH_TRAIN_IMAGES_TEXT}/Flickr_8k.trainImages.txt'
    ...
    # load test set
    filename =  f'{PATH_TRAIN_IMAGES_TEXT}/Flickr_8k.testImages.txt'
    test = load_set(filename)
